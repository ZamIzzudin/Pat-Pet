...
export const PET_ASSETS = {
  DRAGON: {
    ...
    metadata: {
      name: "Dragon",
      description:
        "A fierce dragon companion that breathes fire and soars through the skies",
      rarity: "Epic",
      element: "Fire",
    },
  },
...
export const convertEvolutionStageToString = (
  stage: any
): EvolutionStageString => {
...
export class PetMetadataHelper {
  ...
  static generatePetMetadata(params: PetMetadataParams): GeneratedPetMetadata {
    ...
    const progressPercentage =
      params.totalMilestones > 0
        ? Math.round(
            (params.milestonesCompleted / params.totalMilestones) * 100
          )
        : 0;

    const rarity = this.calculateRarity(
      stageKey,
      params.milestonesCompleted,
      params.totalMilestones
    );
    const mood =
      params.isHappy !== undefined
        ? params.isHappy
          ? "Happy"
          : "Sad"
        : "Neutral";
  ...
  static generateUpdatedPetMetadata(
    ...
    const existingGoalId = currentMetadata.attributes.find(
      (attr) => attr.trait_type === "Goal ID"
    )?.value as number;
    const existingTotalMilestones = currentMetadata.attributes.find(
      (attr) => attr.trait_type === "Total Milestones"
    )?.value as number;
    const existingPetName = currentMetadata.name;
    const existingBirthTime = currentMetadata.attributes.find(
      (attr) => attr.trait_type === "Birth Time"
    )?.value as number;
  ...
  private static calculateRarity(
    ...
    if (stage === "ADULT") {
      ...
      const progress =
        totalMilestones > 0 ? milestonesCompleted / totalMilestones : 0;
  ...
  static async fetchMetadataFromIPFS(ipfsHash: string) {
    try {
      try {
        ...
        console.warn(
          "⚠️ Pinata SDK fetch failed, falling back to direct gateway"
        );
      }
