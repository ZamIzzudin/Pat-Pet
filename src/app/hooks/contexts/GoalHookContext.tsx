...
export function GoalProvider({ children }: { children: React.ReactNode }) {
  ...
  const { data: systemStatsData, refetch: systemStatsDataRefetch } =
    useReadContract({
  ...
  const { data: goalProgressData, refetch: refetchGoalProgress } = useQuery(
    GET_GOAL_PROGRESS,
    {
  ...
  const { data: pendingMilestonesData } = useQuery(
    GET_USER_PENDING_MILESTONES,
    {
      variables: { userAddress: userAddress || "" },
      skip: !userAddress,
  ...
  const validateGoalCreation = (params: CreateGoalParams): string[] => {
    ...
    params.milestoneDescriptions.forEach((desc, index) => {
      ...
      if (desc.length > 200) {
        errors.push(
          `Milestone ${index + 1} description must be less than 200 characters`
        );
      }
    });
  ...
  const getEvolutionPrediction = (milestones: number) => {
    if (milestones >= EVOLUTION_THRESHOLDS.ADULT_THRESHOLD) {
      ...
      return {
        ...
        progressPercentage:
          50 + (milestones - EVOLUTION_THRESHOLDS.BABY_THRESHOLD) * 25,
  ...
  const submitMilestone = async (params: SubmitMilestoneParams) => {
    ...
    const transactionRequest = {
      ...
      writeContract: {
        ...
        args: [params.milestoneId, evidenceIPFS],
      },
      metadata: {
